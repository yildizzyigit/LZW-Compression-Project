...
class LZWDecompressColorImage:
   ...
   def decompress_image_file(self):
      ...
      input_file = os.path.splitext(self.filename)[0] + '_color_compressed.bin'
      input_path = current_directory + '/' + input_file
      output_file = os.path.splitext(self.filename)[0] + '_color_decompressed.bmp'
      ...
      # reconstruct color image from R, G, B channels
      r_array = np.array(r_pixels, dtype=np.uint8).reshape(height, width)
      g_array = np.array(g_pixels, dtype=np.uint8).reshape(height, width)
      b_array = np.array(b_pixels, dtype=np.uint8).reshape(height, width)
...
# decompress the color image
filename = 'small_image.bmp'
lzw = LZWDecompressColorImage(filename)
